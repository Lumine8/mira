...
_CONTINUATION_NOTE = (
    ...
    "voice asked for a real answer, so write structured findings now, not "
    "narration. Format: "
    "**Paper** (First Author, Year) — Journal. 1-2 sentence summary of what the "
    "paper found. Then group the findings by theme, note where they agree or "
    "conflict, and end with a 2-3 sentence verdict on what the evidence overall "
    "says. Use real author names and years from the papers in front of you. "
    "Do not write paragraphs of your own reasoning — just the evidence, "
    "organized. Cover at least fifteen papers when the search returned that many. "
    "Do not propose further searches and do not reintroduce yourself."
)
...
_WEB_CONTINUATION_NOTE = (
    ...
    "so write it now as structured findings, not narration. Format: "
    "**Finding** (source title + URL) — what the source says, in 1-2 sentences. "
    "List every result that has real information. When a snippet is only a hint "
    "and not a real answer, say so and propose the page with "
    "[[browse|the url|why]] so you can read it in full. "
    "Do not write paragraphs of your own thoughts — just the facts from the "
    "sources, with links. Do not reintroduce yourself."
)
